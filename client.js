// dsh-voice-input — browser half (dsh.client web plugin bundle).
//
// 语音输入插件：在输入框工具行提供麦克风按钮，用浏览器 Web Speech API
// 将语音实时转写为文字并写入输入框（可选自动发送），支持 8 种语言。
//
// 要求：Chrome / Edge（Firefox 不支持 Web Speech API）。
...
`;
    ...
    function apply(ctx) {
      // ---- 样式注入（随插件卸载自动清理） ----
      ctx.effect(() => {
        const id = "dsh-voice-input-css";
        let el = document.querySelector('style[data-plugin-css="' + id + '"]');
        if (el === null) {
          el = document.createElement("style");
          el.dataset.pluginCss = id;
          el.dataset.plugin = "dsh-voice-input";
          el.textContent = CSS;
          document.head.appendChild(el);
        }
        return () => {
          if (el !== null && el.parentNode !== null && el.parentNode !== undefined) el.parentNode.removeChild(el);
        };
      });
